        ...
        <div className="container-custom h-full flex items-center justify-between">
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-1.5 text-emerald font-bold uppercase tracking-widest">
              <PhoneCall size={10} />
              <span className="hidden sm:inline">Emergency:</span> <span>{hospital.phones.main}</span>
            </div>
            ...
            {links.map((link) => (
              <Link 
                ...
                className={cn(
                  ...
                  (link.label === "Cardiology" || link.label === "Diagnostics") && "hidden xl:block"
                )}
      ...
      <div className={cn(
        ...
        <div className="flex flex-col min-h-full pt-20 pb-12 px-8">
          ...
          <div className="flex flex-col gap-5 mt-8">
            <span className="text-[10px] font-bold uppercase tracking-[0.4em] text-emerald mb-4">Navigation</span>
            {links.map((link, idx) => (
              <Link 
                key={link.to} 
                to={link.to} 
          ...
          <div className="mt-16 flex flex-col gap-6 text-white/40 border-t border-white/10 pt-10">
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="text-[9px] uppercase font-bold tracking-[0.2em] mb-1">Emergency 24/7</div>
                <div className="text-base font-bold text-white tracking-tight">{hospital.phones.main}</div>
              </div>
            </div>
            <div className="flex items-center gap-4">
              <div className="h-10 w-10 rounded bg-white/5 flex items-center justify-center text-emerald">
              ...
              <div>
                ...
                <span className="text-[11px] leading-relaxed max-w-[220px] block text-white/60 font-medium">{hospital.address.full}</span>
              </div>
            </div>
          </div>
        </div>
