...
export const Route = createFileRoute("/doctors/$slug")({
  ...
  head: ({ loaderData }) => ({
    meta: [
      ...
      { name: "description", content: `${loaderData?.doc.name}, ${loaderData?.doc.title} at Aramana Hospital & Heart Centre, Kasaragod. ${loaderData?.doc.qualifications}.` },
    ],
  }),
});
              ...
              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10 mb-16">
                  <div className="flex items-start gap-6">
                    ...
                    <div>
                      <div className="text-[10px] uppercase tracking-[0.2em] font-bold text-white/40 mb-2">Designation</div>
                  ...
                  <div className="flex items-start gap-6">
                    ...
                    <div>
                      <div className="text-[10px] uppercase tracking-[0.2em] font-bold text-white/40 mb-2">OP Timings</div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-24">
            <div className="lg:col-span-7">
              <div className="mb-20">
                <div className="flex items-center gap-4 mb-10">
                  <span className="h-px w-12 bg-emerald" />
                  <span className="text-emerald font-bold uppercase tracking-[0.3em] text-[10px] md:text-[11px]">Professional Bio</span>
                </div>
                <h2 className="heading-section text-cardiac-blue mb-10">About <span className="text-emerald text-display font-normal">{doc.name}</span></h2>
                <div className="space-y-8 text-editorial">
                  {doc.bio.map((p: string, i: number) => <p key={i}>{p}</p>)}
                </div>
              </div>
              ...
              <div>
                <div className="flex items-center gap-4 mb-10">
                  <span className="h-px w-12 bg-emerald" />
                  <span className="text-emerald font-bold uppercase tracking-[0.3em] text-[10px] md:text-[11px]">Clinical Focus</span>
                </div>
                <h2 className="heading-section text-cardiac-blue mb-10">Expertise & Specialization</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {doc.expertise.map((item: string, idx: number) => (
                    <div key={idx} className="flex items-center gap-4 bg-slate-50 p-6 rounded-xl border border-slate-100 group hover:bg-white hover:shadow-xl transition-all">
                      <CheckCircle2 size={18} className="text-emerald shrink-0" />
                      <span className="text-cardiac-blue font-bold text-[13px] uppercase tracking-wide">{item}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="lg:col-span-5">
              <div className="sticky top-32">
                <div className="bg-slate-50 p-10 md:p-16 rounded-xl border border-slate-100 relative overflow-hidden group">
                  ...
                  <h3 className="text-2xl font-extrabold text-cardiac-blue mb-10 relative z-10">Institutional Affiliations</h3>
                  <div className="space-y-10 relative z-10">
                    <div className="flex gap-6">
                      ...
                      <div>
                        <h4 className="text-cardiac-blue font-extrabold mb-2 text-base">Aramana Heart Centre</h4>
                        <p className="text-muted text-sm leading-relaxed">Chief Consultant in the Department of {doc.department}.</p>
                      </div>
                    </div>
                    <div className="flex gap-6">
                      <div className="h-12 w-12 bg-white rounded-lg shadow-sm flex items-center justify-center text-emerald shrink-0">
                      ...
                      <div>
                        <h4 className="text-cardiac-blue font-extrabold mb-2 text-base">Emergency Support</h4>
                        <p className="text-muted text-sm leading-relaxed">Available for acute cardiac interventions 24/7 through the emergency reception.</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="mt-12 pt-10 border-t border-slate-200 relative z-10">
                    <div className="flex items-center justify-between mb-8">
                      <span className="text-muted text-[10px] font-bold uppercase tracking-[0.2em]">Direct Reception</span>
                      <span className="text-cardiac-blue font-extrabold text-lg tracking-tighter">{hospital.phones.main}</span>
                    </div>
                    <Link 
                      to="/appointment"
